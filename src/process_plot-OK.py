...
## def toProofYamlRead(config: Dict) -> int:
    ########## print(config)
    ...
    ## print(df_name1); print(df_name1.shape); input(11); print(df_name1[0]); input(11)
    df1= pd.read_csv(df_name1[0], index_col=0, low_memory=False)
    ...
    ## 2-4-3-1- scaler/standarizer names-.
    sca= config['scaler']['scaler_'] # used to scale/standarize a WHOLE DF-.
    ...
    ## 2-4-3-2- scaler/standarizer absolute PATH-.
    scaler_file= root_mod_sca+ sca
    scaler_targ_file= root_mod_sca+ sca_targ
    scaler_feat_file= root_mod_sca+ sca_feat
    ## 2-4-3-3 load scaler/standarizer-.
    with open(scaler_file, 'rb') as fs: scaler= pickle.load(fs)
    ...
    if not is_folder_exist:
        ...
        print('The folder ===< {0}{1}{2} >=== FUE CREADO'.format('\t', folder_figs, '\t'))
    else:
        print('The folder ===< {0}{1}{2} >=== EXISTE'.format('\t', folder_figs, '\t'))
    ## ======================================================================= END79
    ...
    df1, n_cols_rem1= gt.remove_cols_zeros(df1)
    ...
    for i in range(n_cases):
        ## print(i)
        irun= np.random.randint(df1['irun'].min(), df1['irun'].max())
        iruned.append(irun)
        ## https://stackoverflow.com/questions/5036700/how-can-you-dynamically-create-variables
        locals()['df_{0}'.format(irun)]= df1[df1['irun']==irun]
        df_models.append(str('df_{0}'.format(irun)))

    case= '_out'
    df= df1.loc[:,feature_in_var]
    in_features= [col for col in df1.columns if '_in' in col or 'vgrad' in col] # features -.
    out_targets= [col for col in df1.columns if '_out' in col] # targets -.
    ...
    for ig, df_model in enumerate(df_models):
        ...
        df_recur_1= gt.pred_iter_pred_mod(df_temp, feat_vars, target_var, scaler, scaler_feat, scaler_targ, model)
        end_time= time.time()
        print('Execution_time {0}'.format(end_time-start_time))
        ...
        plt.show()
        fig.savefig(os.path.join(folder_figs,'Cijkl_teoVsCijkl_Pred-RecurVsStrain_'+
